...
namespace animeAlley.Controllers
{
    public class ShowsController : Controller
    {
        ...
        // POST: Shows/Edit/5Add commentMore actions
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (!string.IsNullOrWhiteSpace(show.Imagem))
            {
                var pastaImagens = Path.Combine(
                    _webHostEnvironment.WebRootPath,
                    "images", "showCover"
                );
                var caminhoImagem = Path.Combine(pastaImagens, show.Imagem);

                if (System.IO.File.Exists(caminhoImagem))
                    System.IO.File.Delete(caminhoImagem);
            }

            if (!string.IsNullOrWhiteSpace(show.Banner))
            {
                var pastaImagens = Path.Combine(
                    _webHostEnvironment.WebRootPath,
                    "images", "showBanner"
                );
                var caminhoImagem = Path.Combine(pastaImagens, show.Banner);
